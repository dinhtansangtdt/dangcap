...
def parse_args():
    ...
    parser.add_argument(
        "--skip-activation",
        action="store_true",
        help="Bỏ qua quy trình kích hoạt, khởi động ứng dụng trực tiếp (chỉ dùng để gỡ lỗi)",
    )
...
async def start_app(mode: str, protocol: str, skip_activation: bool) -> int:
    ...
    """
    logger.info("Khởi động ứng dụng khách AI Tiểu Trí")

    # 处理激活流程
    if not skip_activation:
        activation_success = await handle_activation(mode)
        if not activation_success:
            logger.error("Kích hoạt thiết bị thất bại, chương trình thoát")
            return 1
    else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Chương trình bị người dùng ngắt")
        exit_code = 0
    except Exception as e:
