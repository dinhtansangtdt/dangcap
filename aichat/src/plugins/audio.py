    ...
    def _should_send_microphone_audio(self) -> bool:
        ...
        try:
            ...
            if self._in_silence_period:
                return False
            should_send = self.app and self.app.should_capture_audio()
            # Debug log để kiểm tra
            if should_send and self.app:
                logger.debug(f"[AUDIO] Gửi audio, state={self.app.device_state}, channel_open={self.app.is_audio_channel_opened()}")
            return should_send
        except Exception:
            return False
