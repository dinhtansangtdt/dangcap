...
class ActivationWindow(BaseWindow, AsyncMixin):
    ...
    def _show_activation_info(self, activation_data: dict):
        ...
        # 信息已在UI界面显示，仅记录简要日志
        self.logger.info(f"Lấy mã xác thực kích hoạt: {code}")
    ...
    def _on_retry_clicked(self):
        ...
        跳转激活按钮点击 - 打开激活网页.
        """
        self.logger.info("Người dùng nhấp chuyển đến kích hoạt")
        ...
        try:
            ...
            if ota_url:
                ...
                self.logger.error("Chưa cấu hình URL kích hoạt")
                self.update_status("Lỗi: Chưa cấu hình URL kích hoạt")
        except Exception as e:
            self.logger.error(f"Không thể mở trang kích hoạt: {e}")
            self.update_status(f"Không thể mở trang kích hoạt: {e}")
